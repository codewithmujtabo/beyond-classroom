...
export function AuthProvider({
  ...
  async function login(email: string, password: string) {
    try {
      ...
      // T2.2 — Register push token after successful login
      if (pushToken) {
        try {
          await pushService.registerPushToken(pushToken);
        } catch (err) {
          console.warn("Failed to register push token:", err);
          // Non-fatal — don't block login if push registration fails
        }
      }
    } catch (err: any) {
  ...
  async function signup(params: {
    ...
    try {
      ...
      // T2.2 — Register push token after successful signup
      if (pushToken) {
        try {
          await pushService.registerPushToken(pushToken);
        } catch (err) {
          console.warn("Failed to register push token:", err);
          // Non-fatal — don't block signup if push registration fails
        }
      }
    } catch (err: any) {
  ...
  async function logout() {
    try {
      ...
      // T2.5 — Clear push token on logout
      if (pushToken) {
  ...
  // T2.2, T2.4 — Handle push token registration
  async function setPushToken(token: string | null) {

...
router.post("/webhook", async (req: Request, res: Response) => {
  try {
    ...
    // ── Look up the payment record with user and competition details ──────
    ...
    const { id: paymentDbId, registration_id, user_id, competition_name } = paymentResult.rows[0];
    ...
    if (isSuccess) {
      // T4.2 — Payment successful
      await pushService.sendPushNotification(
        user_id,
        "Payment Confirmed!",
        `Your payment for ${competition_name} has been confirmed.`,
        { type: "payment_success", compId: registration_id, registrationId: registration_id }
      ).catch(err => console.warn("Failed to send payment success notification:", err));
    } else if (transaction_status === "pending") {
      // T4.3 — Payment pending
      await pushService.sendPushNotification(
        user_id,
        "Payment Pending",
        `Complete your payment for ${competition_name}.`,
        { type: "payment_pending", compId: registration_id, registrationId: registration_id }
      ).catch(err => console.warn("Failed to send payment pending notification:", err));
    } else if (["deny", "cancel", "expire", "failure"].includes(transaction_status)) {
      // T4.4 — Payment failed
      await pushService.sendPushNotification(
        user_id,
        "Payment Failed",
        `Payment failed for ${competition_name}. Please try again.`,
        { type: "payment_failed", compId: registration_id, registrationId: registration_id }
      ).catch(err => console.warn("Failed to send payment failed notification:", err));
    }

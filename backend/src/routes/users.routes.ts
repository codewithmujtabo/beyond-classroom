...
// Register or update user's Expo Push Token for notifications (T2.3)
router.post("/push-token", async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ message: err.message || "Failed to register push token" });
  }
});

// ── DELETE /api/users/push-token ──────────────────────────────────────────────
// Clear user's push token (called on logout) (T2.5)
router.delete("/push-token", async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ message: err.message || "Failed to clear push token" });
  }
});
